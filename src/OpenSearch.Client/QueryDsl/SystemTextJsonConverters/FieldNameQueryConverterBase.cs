...
namespace OpenSearch.Client.QueryDsl.SystemTextJsonConverters
{
		...
		where TConcrete : class, TQuery, new()
	{
		...
		public override void Write(Utf8JsonWriter writer, TQuery value, JsonSerializerOptions options)
		{
			...
			if (field == null)
				throw new JsonException("Field name cannot be null for a field-name query");

			// Use FieldConverter.WriteAsPropertyName to resolve the field name correctly
			// (handles expressions, property mappings, camelCase conversion, etc.)
			var fieldConverter = GetFieldConverter(options);
			if (fieldConverter != null)
				fieldConverter.WriteAsPropertyName(writer, field, options);
			else
				writer.WritePropertyName(field.Name ?? field.ToString());
		...
		private static FieldConverter GetFieldConverter(JsonSerializerOptions options)
		{
