...
namespace OpenSearch.Client.QueryDsl.SystemTextJsonConverters
{
	...
	internal sealed class MatchQueryConverter : FieldNameQueryConverterBase<IMatchQuery, MatchQuery>
	{
		...
		private static void WriteFuzziness(Utf8JsonWriter writer, IFuzziness fuzziness)
		{
			if (fuzziness.Auto)
			{
				if (!fuzziness.Low.HasValue || !fuzziness.High.HasValue)
					writer.WriteStringValue("AUTO");
				else
					writer.WriteStringValue($"AUTO:{fuzziness.Low},{fuzziness.High}");
			}
			else if (fuzziness.EditDistance.HasValue)
			{
				writer.WriteNumberValue(fuzziness.EditDistance.Value);
			}
			else if (fuzziness.Ratio.HasValue)
			{
				writer.WriteNumberValue(fuzziness.Ratio.Value);
			}
			else
			{
