...
{
	/// <summary>
	/// Polymorphic converter for <see cref="IProperty"/> and concrete property types.
	...
	/// </summary>
	internal sealed class PropertyConverterFactory : JsonConverterFactory
	{
		public override bool CanConvert(Type typeToConvert) =>
			typeof(IProperty).IsAssignableFrom(typeToConvert);

		public override JsonConverter CreateConverter(Type typeToConvert, JsonSerializerOptions options)
		{
			if (typeToConvert == typeof(IProperty))
				return new PropertyInterfaceConverter();

			// For concrete types (TextProperty, KeywordProperty, etc.), use a converter
			// that writes the "type" discriminator field
			var converterType = typeof(ConcretePropertyConverter<>).MakeGenericType(typeToConvert);
			return (JsonConverter)System.Activator.CreateInstance(converterType);
		}
	}

	internal sealed class ConcretePropertyConverter<T> : JsonConverter<T> where T : class, IProperty
	{
		public override T Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
		{
			...
			// Use default deserialization but through a temporary options that doesn't include this converter
			// to avoid infinite recursion. We parse the JSON element and reconstruct.
			using var doc = JsonDocument.ParseValue(ref reader);
			var rawJson = doc.RootElement.GetRawText();

			// Create a copy of options without this factory to avoid recursion
			var tempOptions = new JsonSerializerOptions(options);
			RemovePropertyConverterFactory(tempOptions);
			return JsonSerializer.Deserialize<T>(rawJson, tempOptions);
		}
		...
		public override void Write(Utf8JsonWriter writer, T value, JsonSerializerOptions options)
		{
			...
			// Serialize to a temporary document, then rewrite with "type" added
			using var tempDoc = JsonSerializer.SerializeToDocument(value, tempOptions);
			writer.WriteStartObject();

			// Always write "type" first
			var typeValue = ((IProperty)value).Type;
		...
		private static void RemovePropertyConverterFactory(JsonSerializerOptions options)
		{
			for (int i = options.Converters.Count - 1; i >= 0; i--)
			{
				if (options.Converters[i] is PropertyConverterFactory)
				{
